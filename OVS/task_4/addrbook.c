...
struct list_element *list_new_elem (struct address *addr)
 {
  //if(!addr) { return NULL; } // fixed NULL pointer dereference
...
void list_append (struct list *list, struct address *addr)
 {
  ...
  if (!list) { //list == NULL: release addr if allocated
//    if (addr) {
...
int main (int argc, char **argv)
 {
  ...
  /* main loop */
  #ifdef __VERIFY
  while (c != 'q' && main_counter < 3) { 
    main_counter++; //(for unwinding assert)*/
  #else
  while (c != 'q') {
  #endif
